...
@chat_router.get("/api/debug/ip",
    summary="Debug IP headers",
    description="Returns the IP Traefik resolved after applying trustedIPs. Only available in non-production.",
    tags=["System"])
async def debug_ip(request: Request):
    """
    Shows the real client IP as seen by the app after Traefik processes it.

    How to verify trustedIPs is working:
    - client.host should be your REAL public IP (not a Cloudflare IP like 173.245.x.x)
    - X-Forwarded-For should contain your real IP
    - CF-Connecting-Ip should match your real IP (added by Cloudflare)
    ...
    return {
        ...
        "headers": {
            ...
            "CF-IPCountry":      request.headers.get("Cf-Ipcountry", "not set"),
        },
        "verdict": (
            "✅ WORKING — Traefik is reading your real IP"
            if request.client and not _is_cloudflare_ip(request.client.host)
            else "❌ NOT WORKING — Traefik is still seeing Cloudflare's IP"
        )
    }
